  ...
  return (
    <div className="space-y-6">
      <Card title={`Identität${userName ? ` — ${userName}` : ""}`}>
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <div className="rounded-lg border border-zinc-800 bg-zinc-900 px-3 py-2 text-sm text-zinc-200">
      ...
      <Card title="Bonität (Selbstauskunft)">
        ...
        <div className="mt-5 grid gap-3 rounded-xl border border-zinc-800 bg-zinc-950/40 p-4 md:grid-cols-3">
          <Stat
...
function Label({ children }: { children: React.ReactNode }) {
  return <div className="mb-1 text-xs uppercase tracking-wide text-zinc-400">{children}</div>;
}
  ...
  return (
    <Card title={`Trackrecord (${items.length})`}>
      <div className="space-y-3">
            ...
            {items.map((it) => (
              <div key={it.id} className="flex flex-wrap items-start justify-between gap-3 py-3">
                ...
                <button
                  ...
                  className="rounded-lg border border-zinc-800 px-3 py-1 text-xs text-zinc-300 hover:border-rose-500 hover:text-rose-300"
                >
                  Löschen
                </button>
              </div>
        ...
        {open ? (
          <form onSubmit={add} className="grid gap-3 rounded-xl border border-zinc-800 bg-zinc-950/40 p-4 md:grid-cols-3">
            <div>
